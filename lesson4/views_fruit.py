from models_fruit import Base, User, Product
...
def showCategoriedProducts(category):
    ...
    items=session.query(Product).filter_by(category=category).all()
    return jsonify(produce_products=[p.serialize for p in items])
